...
export default function ReasonPlatformNotHelpfulModal({
  form,
  setForm,
  nextStep,
  prevStep,
  ...
  const [feedback, setFeedback] = useState(form.platformFeedback ?? "");

  // Only enable if feedback is not just whitespace
  const canContinue = feedback.trim() !== "";

  const handleContinue = () => {
    if (!canContinue) return;
    setForm(f => ({ ...f, platformFeedback: feedback }));
    nextStep();
  };
      ...
      <div className="relative flex flex-row rounded-2xl bg-white p-10 min-w-[800px] max-w-[900px]">
        ...
        <div className="flex-1 pr-10 flex flex-col justify-center">
          ...
          <div className="flex items-center mb-4">
            ...
            <span className="font-medium text-lg">Platform not helpful</span>
          </div>
          ...
          <div className="flex flex-col gap-4">
            <button
              className={`bg-green-500 text-white py-4 rounded-lg text-lg font-semibold flex justify-center items-center hover:bg-green-600 transition-colors ${canContinue ? "" : "opacity-50 cursor-not-allowed"}`}
